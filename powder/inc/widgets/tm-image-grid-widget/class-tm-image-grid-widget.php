...
if ( !class_exists( 'Powder_Image_Grid_Widget' ) ) {

	/**
	 * Image Grid Widget
	 */
	class Powder_Image_Grid_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name			= esc_html__( 'Image Grid Widget', 'powder' );
			$this->widget_description	= esc_html__( 'This widget displays images from post.', 'powder' );
			...
			$this->settings				= array(
				...
				'category_name' => array(
					...
					'label'				=> esc_html__( 'Select categories to show', 'powder' ),
				),
				'tag' => array(
					'type'				=> 'select',
					'multiple'			=> true,
				...
				'post_sort' => array(
					...
					'value'				=> 'date',
					'options'		=> array(
						'date' 				=> esc_html__( 'Publish Date', 'powder' ),
						'title'				=> esc_html__( 'Post Title', 'powder' ),
						'comment_count'		=> esc_html__( 'Comment Count', 'powder' ),
					),
					'label'				=> esc_html__( 'Post sorted', 'powder' ),
				),
				'post_number' => array(
					'type'				=> 'stepper',
					'value'				=> '5',
					...
					'min_value'			=> '1',
					'step_value'		=> '1',
					'label'				=> esc_html__( 'Posts number', 'powder' ),
				),
				'post_offset' => array(
					'type'				=> 'stepper',
					'value'				=> '0',
					...
					'min_value'			=> '0',
					'step_value'		=> '1',
					'label'				=> esc_html__( 'Offset post', 'powder' ),
				),
				'title_length' => array(
					'type'				=> 'stepper',
					'value'				=> '10',
					...
					'min_value'			=> '0',
					'step_value'		=> '1',
					'label'				=> esc_html__( 'Title words length ( Set 0 to hide title. )', 'powder' ),
				),
		...
		public function get_terms_list ( $tax = 'category' ) {
			...
			if ( $terms ){
				foreach ( $terms as $term ) {
					$output_terms[ $term->slug ] = $term->name /*. sprintf( _n( ' ( 1 post )', ' ( %s posts )', $term->count, 'powder' ), $term->count )*/;
				}
			}
		...
		public function get_post_title( $post, $instance ) {
			...
			if( '0' !== $instance['title_length'] ){
				$title = wp_trim_words( $post->post_title, $instance['title_length'], esc_html__( ' ...', 'powder' ) );
			}
		...
		public function get_post_image( $post, $image_size ) {
			...
			if( !$image ){
				...
				$placeholder_attr = apply_filters( 'powder_image_grid_widget_placeholder_default_args',
					array(
						'width'			=> $size['width'],
						'height'		=> $size['height'],
						'background'	=> '000',
		...
		public function widget( $args, $instance ) {
			...
			$args = apply_filters( 'powder_image_grid_widget_args', $args );
			...
			if ( isset( $posts ) && $posts ) {
				...
				echo apply_filters( 'powder_image_grid_widget_before', '<div class="row columns-number-' . $columns_number . '" ' . $row_inline_style . '>' );

				foreach ( $posts as $post ) {
					setup_postdata( $post );

					$image = $this->get_post_image( $post, apply_filters( 'powder_image_grid_widget_size', '_tm-thumb-m' ) );
				...
				echo apply_filters( 'powder_image_grid_widget_after', '</div>' );
			}
	...
	add_action( 'widgets_init', 'powder_register_image_grid_widget' );
	function powder_register_image_grid_widget() {
		register_widget( 'powder_Image_Grid_Widget' );
	}
