...
class __Tm_Custom_Posts_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		$this->widget_name = esc_html__( 'Custom Posts', 'jane_style' );
		$this->widget_description = esc_html__( 'Display custom posts your site.', 'jane_style' );
		$this->widget_id = apply_filters( 'jane_style_custom_posts_widget_ID', 'widget-custom-postson' );
		$this->widget_cssclass = apply_filters( 'jane_style_custom_posts_widget_cssclass', 'widget-custom-postson' );

		$this->settings = array(
			'title' => array(
				'type'				=> 'text',
				'value'				=> esc_html__( 'Custom Posts', 'jane_style' ),
			...
			'terms_type' => array(
				...
				'options'			=> array(
					...
					'post_format'		=> array(
						'label'		=> esc_html__( 'Post Format', 'jane_style' ),
			...
			'post_format' => array(
				...
				'options'			=> false,
				'label'				=> esc_html__( 'Select post format', 'jane_style' ),
				'multiple'			=> true,
				'placeholder'		=> esc_html__( 'Select post format', 'jane_style' ),
			...
			'posts_per_page' => array(
				...
				'max_value'			=> 50,
				'min_value'			=> 0,
				'label'				=> esc_html__( 'Posts count ( Set 0 to show all. )', 'jane_style' ),
			),
			'post_offset' => array(
				'type'				=> 'stepper',
				'value'				=> '0',
			...
			'excerpt_length' => array(
				...
				'min_value'			=> '0',
				'step_value'		=> '1',
				'label'				=> esc_html__( 'Excerpt words length ( Set 0 to hide excerpt. )', 'jane_style' ),
			),
			'mate_data' => array(
				'type'				=> 'checkbox',
				'value'				=> array(
				...
				'options'				=> array(
					'date'				=> esc_html__( 'Date', 'jane_style' ),
					'author'			=> esc_html__( 'Author', 'jane_style' ),
					'comment_count'		=> esc_html__( 'Comment count', 'jane_style' ),
					'category'			=> esc_html__( 'Category', 'jane_style' ),
					'post_tag'			=> esc_html__( 'Tag', 'jane_style' ),
					'more_button'		=> esc_html__( 'More Button', 'jane_style' ),
				),
				'label'				=> esc_html__( 'Display post meta data', 'jane_style' ),
			),
			'button_text' => array(
				'type'				=> 'text',
				'value'				=> 'Read More',
				'label'				=> esc_html__( 'Post read more button label', 'jane_style' ),
			),
		);
	...
	public function cut_text( $text, $length ) {
		return ( '0' !== $length ) ? wp_trim_words( $text, $length, esc_html__( ' ...', 'jane_style' ) ) : '' ;
	}
	...
	public function get_post_date( $visible = 'true' ) {
		if ( $visible === 'true') {
			...
			$date_icon = apply_filters( 'jane_style_custom_posts_date_icon', '<i class="material-icons">schedule</i>' );
			$permalink = $this->get_post_permalink();
	...
	public function get_the_author( $user_id, $visible = 'true' ) {
		if ( $visible === 'true' ) {
			...
			$html= '<div class="post-author">' . esc_html__( 'by ', 'jane_style' ) . $author . '</div>';
	 ...
	 * @return string
	 */
	public function get_post_image( $post, $image_size = 'jane_style-thumb-m', $image_mobile_size = 'jane_style-thumb-s' ) {
		...
		if( $post_thumbnail_id ){
			...
			$placeholder_attr = apply_filters( 'jane_style_custom_posts_placeholder_default_args',
				array(
					'width'			=> $size['width'],
					'height'		=> $size['height'],
					'background'	=> get_theme_mod( 'regular_accent_color_1', jane_style_theme()->customizer->get_default( 'regular_accent_color_1' ) ),
					'foreground'	=> get_theme_mod( 'regular_accent_color_2', jane_style_theme()->customizer->get_default( 'regular_accent_color_2' ) ),
	...
	public function get_button( $text = '', $visible = 'true' ){
		if ( $visible === 'true' ) {
			$permalink = $this->get_post_permalink();
			$icon = apply_filters( 'jane_style_custom_posts_read_more_icon', '' ); //<span class="btn__icon"></span>
	...
	public function widget( $args, $instance ) {
		...
		if ( $posts ) {
			...
			$image_size = apply_filters( 'jane_style_playlist_thumbnail_image_size', 'jane_style-thumb-m' );
			$image_mobile_size = apply_filters( 'jane_style_playlist_thumbnail_size_mobile', 'jane_style-thumb-s' );
...
add_action( 'widgets_init', 'jane_style_register_custom_posts_widget' );
function jane_style_register_custom_posts_widget() {
