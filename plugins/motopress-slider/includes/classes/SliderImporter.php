			...
			if (check_admin_referer('mpsl-import', 'mpsl-import-nonce')) {
				...
				if (false !== $data) {
					...
					if ($importResult && $action === 'preset') {
						...
						echo '<br/><a href="' . $sliderUrl . '" class="button button-primary">' . __('Go to slider settings page', MPSL_TEXTDOMAIN) . '</a>';
...
//                $slider->overrideOptions($slider_data['options'], false);
        ...
        if ($action === 'preset') {
	        if (!is_null($slider)) {
				...
				printf(__('Slider preset import error: %s', MPSL_TEXTDOMAIN), $wpdb->last_error);
				echo '<br/>';
		        return false;
	        }
        }
