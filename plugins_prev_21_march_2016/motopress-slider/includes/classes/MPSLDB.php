...
//        return wp_get_attachment_url($post_thumbnail_id, 'full');
    ...
    public function getExcerpt($post, $charLength) {
        $excerpt = $post->post_content;
//        $excerpt = apply_filters('the_excerpt', $post->post_excerpt);
        $rawExcerpt = $excerpt;
	    $excerpt = apply_filters('the_content', $excerpt);
	    $excerptMore = apply_filters('excerpt_more', ' ...');
        $excerpt = strip_tags($this->stripShortcodes($excerpt));
        $text = $this->getTrimmedString($excerpt, $charLength, $excerptMore);

        return apply_filters('wp_trim_excerpt', $text, $rawExcerpt);
    }

    private function getTrimmedString($string, $maxLength = 10000, $more = ' ...') {
        $maxLength = $maxLength > 0 ? $maxLength : 10000;
        return function_exists('mb_strimwidth') ? mb_strimwidth($string, 0, $maxLength, $more) : substr($string, 0, $maxLength) . $more;
    }
