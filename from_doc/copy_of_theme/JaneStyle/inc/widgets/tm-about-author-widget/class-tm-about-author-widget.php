<?php
/**
 * Widget jane_style about author
 *
 * @package jane_style
 */

class Jane_Style_About_Author_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_cssclass    = 'jane_style widget-about-author';
		$this->widget_description = __( 'Display an information about selected user.', 'jane_style' );
		$this->widget_id          = 'jane_style_widget_about_author';
		$this->widget_name        = __( 'About Author', 'jane_style' );
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',
				'value' => __( 'About Author', 'jane_style' ),
				'label' => __( 'Title', 'jane_style' ),
			),
			'user_id' => array(
				'type'             => 'select',
				'size'             => 1,
				...
				'options'          => false,
				'label'            => __( 'Select user to show', 'jane_style' ),
			),
			'avatar_size' => array(
				'type'       => 'slider',
				'max_value'  => 512,
				...
				'value'      => 250,
				'step_value' => 1,
				'label'      => __( 'Author avatar size (set 0 to hide avatar, applied only for Gravatar)', 'jane_style' ),
			),
			'avatar_img' => array(
				'type'               => 'media',
				'value'              => '',
				'multi_upload'       => false,
				'library_type'       => 'image',
				'upload_button_text' => __( 'Select image', 'jane_style' ),
				'label'              => __( 'Custom avatar image (override default user avatar)', 'jane_style' ),
			),
			'link' => array(
				'type'  => 'text',
				'value' => '',
				'label' => __( 'Link (leave empty to hide)', 'jane_style' ),
			),
			'link_text' => array(
				'type'  => 'text',
				'value' => __( 'Read More', 'jane_style' ),
				'label' => __( 'Link label', 'jane_style' ),
			),
		);
	...
	public function get_users_list() {
		...
		$result = array( '0' => __( 'Select a user', 'jane_style' ) );
...
add_action( 'widgets_init', 'jane_style_register_about_author_widgets' );
function jane_style_register_about_author_widgets() {
	register_widget( 'jane_style_About_Author_Widget' );
}
