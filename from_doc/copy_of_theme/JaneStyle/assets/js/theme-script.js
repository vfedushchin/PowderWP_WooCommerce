(function ($) {
	...
	CherryJsCore.theme_script = {
		...
		smart_slider_init: function (self) {
			$('.jane_style-smartslider').each(function () {
				var slider = $(this),
				...
				if ($('.jane_style-smartslider__slides', '#' + sliderId).length > 0) {
					$('#' + sliderId).sliderPro({
						width: sliderWidth,
						height: sliderHeight,
		...
		swiper_carousel_init: function (self) {
			...
			jQuery('.jane_style-carousel').each(function () {
				var swiper = null,
		...
		subscribe_init: function (self) {
			CherryJsCore.variable.$document.on('click', '.subscribe-block__submit', function (event) {
				...
				var $this = $(this),
					...
					nonce = form.find('input[name="jane_style_subscribe"]').val(),
				...
				$.ajax({
					url: jane_style.ajaxurl,
					type: 'post',
					dataType: 'json',
					data: {
						action: 'jane_style_subscribe',
						mail: mail,
						nonce: nonce
					},
					error: function () {
		...
		to_top_init: function (self) {
			if ($.isFunction(jQuery.fn.UItoTop)) {
				$().UItoTop({
					text: jane_style.labels.totop_button,
					scrollSpeed: 600
				});
			}
		}
