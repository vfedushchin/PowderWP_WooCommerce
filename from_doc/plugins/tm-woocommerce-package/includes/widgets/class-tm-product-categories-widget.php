...
class __TM_Product_Categories_Widget extends WC_Widget_Product_Categories {
	...
	public function widget( $args, $instance ) {
		...
		$between = apply_filters( 'tm_categories_carousel_widget_spece_between_slides', 30 );
		...
		if($arrows){
			$end_html[] = '<div id="swiper-carousel-'. $uniqid . '-next" class="swiper-button-next tm-categories-carousel-widget-button-next"><i class="material-icons">navigate_next</i></div>';
			$end_html[] = '<div id="swiper-carousel-'. $uniqid . '-prev" class="swiper-button-prev tm-categories-carousel-widget-button-prev"><i class="material-icons">navigate_before</i></div>';
		}
		$end_html[] = '</div>';
		echo implode ( "\n", $end_html );
	...
	public function start_el( &$output, $cat, $depth = 0, $args = array(), $current_object_id = 0 ) {
		...
		if( '0' !== $thumbnail_id ) {
			...
			if( ( $iPod || $iPhone || $iPad ) && $version ){
				...
				$image = '<img src="' . $image[0] . '" alt="' . $cat_name . '">';
			} else {
				$image = wp_get_attachment_image( $thumbnail_id, apply_filters( 'tm_categories_carousel_widget_thumb_size', 'shop_catalog' ) );
			}
		...
		$html[] = '<a class="tm-categories-carousel-widget-link" href="' . get_term_link( (int) $cat->term_id, 'product_cat' ) . '">';
		$html[] = $image;
