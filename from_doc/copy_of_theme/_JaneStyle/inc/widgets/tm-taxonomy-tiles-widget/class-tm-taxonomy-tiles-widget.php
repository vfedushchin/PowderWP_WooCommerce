...
if ( !class_exists( 'Jane_Style_Taxonomy_Tiles_Widget' ) ) {

	/**
	 * Taxonomy Tiles Widget
	 */
	class Jane_Style_Taxonomy_Tiles_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name			= esc_html__( 'Taxonomy Tiles Widget', 'jane_style' );
			$this->widget_description	= esc_html__( 'This widget displays images from taxonomy.', 'jane_style' );
			...
			$this->settings = array(
				...
				'category'=> array(
					...
					'label'				=> esc_html__( 'Select category to show', 'jane_style' ),
				),
				'post_tag' => array(
					'type'				=> 'select',
					'multiple'			=> true,
				...
				'description_length' => array(
					...
					'min_value'			=> '0',
					'step_value'		=> '1',
					'label'				=> esc_html__( 'Description words length ( Set 0 to hide description. )', 'jane_style' ),
				),
				'show_post_count' => array(
					...
					'options'		=> array(
						'show_post_count_check' => esc_html__( 'Show post count', 'jane_style' ),
					),
				),
				...
				'first_item' => array(
					...
					'options'		=> array(
						'first_item_check' => esc_html__( 'Tiles first item', 'jane_style' ),
					),
				),
				'items_padding' => array(
					'type'				=> 'stepper',
		...
		public function get_terms_list ( $tax = 'category', $args = array( 'hide_empty' => 0, 'hierarchical' => 0 ) ) {
			...
			if ( $terms ){
				foreach ( $terms as $term ) {
					$output_terms[ $term->term_id ] = $term->name . sprintf( _n( ' ( 1 post )', ' ( %s posts )', $term->count, 'jane_style' ), $term->count );
				}
			}
		...
		public function get_term_post_count( $term, $visible ) {
			return ( 'false' === $visible ) ? '' : '<span class="post-count">' . sprintf( _n( '1 post', '%s posts', $term->count, 'jane_style' ), $term->count ) . '</span>';
		}
		...
		public function get_term_description( $term, $lenght ) {
			...
			return ( 0 === $lenght || !$term->description ) ? '' : '<p class="post-desc">' . wp_trim_words( $term->description, $lenght, esc_html__( ' ...', 'jane_style' ) ) . '</p>';
		}
		...
		public function get_term_image( $term, $image_size ) {
			...
			if( $image ){
				...
				$placeholder_attr = apply_filters( 'jane_style_taxonomy_tiles_widget_placeholder_default_args',
					array(
						'width'			=> $size['width'],
						'height'		=> $size['height'],
						'background'	=> '000',
		...
		public function widget( $args, $instance ) {
			...
			if ( isset( $terms ) && $terms ) {
				...
				$tiles_item_number = apply_filters( 'jane_style_taxonomy_tiles_item_number', 0 );
				...
				echo apply_filters( 'jane_style_taxonomy_tiles_widget_before', '<div class="row grid-columns columns-number-' . $columns_number . '" ' . $row_inline_style . '>' );
				...
				foreach ( $terms as $term_key => $term ) {
					...
					$image = $this->get_term_image( $term, apply_filters( 'jane_style_taxonomy_tiles_widget_size', '_tm-thumb-m' ) );
				...
				echo apply_filters( 'jane_style_taxonomy_tiles_widget_after', '</div>' );
			}
	...
	add_action( 'widgets_init', 'jane_style_register_taxonomy_tiles_widget' );
	function jane_style_register_taxonomy_tiles_widget() {
		register_widget( 'jane_style_Taxonomy_Tiles_Widget' );
	}
