<?php
/**
 * Widget powder about author
 *
 * @package powder
 */

class Powder_About_Author_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_cssclass    = 'powder widget-about-author';
		$this->widget_description = __( 'Display an information about selected user.', 'powder' );
		$this->widget_id          = 'powder_widget_about_author';
		$this->widget_name        = __( 'About Author', 'powder' );
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',
				'value' => __( 'About Author', 'powder' ),
				'label' => __( 'Title', 'powder' ),
			),
			'user_id' => array(
				'type'             => 'select',
				'size'             => 1,
				...
				'options'          => false,
				'label'            => __( 'Select user to show', 'powder' ),
			),
			'avatar_size' => array(
				'type'       => 'slider',
				'max_value'  => 512,
				...
				'value'      => 250,
				'step_value' => 1,
				'label'      => __( 'Author avatar size (set 0 to hide avatar, applied only for Gravatar)', 'powder' ),
			),
			'avatar_img' => array(
				'type'               => 'media',
				'value'              => '',
				'multi_upload'       => false,
				'library_type'       => 'image',
				'upload_button_text' => __( 'Select image', 'powder' ),
				'label'              => __( 'Custom avatar image (override default user avatar)', 'powder' ),
			),
			'link' => array(
				'type'  => 'text',
				'value' => '',
				'label' => __( 'Link (leave empty to hide)', 'powder' ),
			),
			'link_text' => array(
				'type'  => 'text',
				'value' => __( 'Read More', 'powder' ),
				'label' => __( 'Link label', 'powder' ),
			),
		);
	...
	public function get_users_list() {
		...
		$result = array( '0' => __( 'Select a user', 'powder' ) );
...
add_action( 'widgets_init', 'powder_register_about_author_widgets' );
function powder_register_about_author_widgets() {
	register_widget( 'powder_About_Author_Widget' );
}
