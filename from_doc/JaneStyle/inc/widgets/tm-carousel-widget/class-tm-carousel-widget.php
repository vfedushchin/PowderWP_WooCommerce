...
class Jane_Style_Carousel_Widget extends Cherry_Abstract_Widget {

	/**
	 * Constructor
	 */
	public function __construct() {
		$this->widget_cssclass = 'jane_style widget_carousel';
		$this->widget_description = esc_html__( 'Display a list of your posts on your site.', 'jane_style' );
		$this->widget_id = 'widget_carousel';
		$this->widget_name = esc_html__( 'Carousel Widget', 'jane_style' );
		$this->settings = array(
			'title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Carousel', 'jane_style' ),
			...
			'slides_per_view' => array(
				...
				'min_value'	=> 1,
				'value'		=> 5,
				'label' 	=> esc_html__( 'Number of slides per view', 'jane_style' ),
			),
			'slides_per_group' => array(
				...
				'min_value'	=> 1,
				'value'		=> 1,
				'label' 	=> esc_html__( 'Number slides per group', 'jane_style' ),
			),
			'slides_per_column' => array(
				...
				'min_value'	=> 1,
				'value'		=> 1,
				'label' 	=> esc_html__( 'Multi Row Slides Layout', 'jane_style' ),
			),
			'space_between_slides' => array(
				...
				'min_value'	=> 0,
				'value'		=> 30,
				'label' 	=> esc_html__( 'Width of the space between slides(px)', 'jane_style' ),
			),
			'duration_speed' => array(
				...
				'min_value'	=> 100,
				'value'		=> 500,
				'label' 	=> esc_html__( 'Duration of transition between slides (ms)', 'jane_style' ),
			),
			'navigation' => array(
				'type'		=> 'switcher',
				'value'		=> 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' 	=> esc_html__( 'Slider navigation', 'jane_style' ),
			),
			'pagination' => array(
				'type'		=> 'switcher',
				'value'		=> 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' 	=> esc_html__( 'Slider pagination', 'jane_style' ),
			),
		);
	...
	public function widget( $args, $instance ) {
		 ...
		 */
		$default_settings = apply_filters( 'jane_style_carousel_default_settings', array() );
		...
		$footer_widget_columns = get_theme_mod( 'footer_widget_columns', jane_style_theme()->customizer->get_default( 'footer_widget_columns' ) );
		...
		$data_attr_line = 'class="jane_style-carousel swiper-container"';
	...
	public function get_query_slider_items( $query_args = array() ) {
		...
		$defaults_query_args = apply_filters( 'jane_style_carousel_default_query_args', array(
			'post_type'      => 'post',
			'orderby'        => 'date',
			'order'          => 'DESC',
			'posts_per_page' => -1,
		...
		$html = '';
			if ( $posts_query->have_posts() ) {
				while ( $posts_query->have_posts() ) : $posts_query->the_post();
					...
					$placeholder_args = apply_filters( 'jane_style_carousel_placeholder_args',
						array(
							'width'			=> 560,
							'height'		=> 350,
							'background'	=> '000',
					...
					$image_size = apply_filters( 'jane_style_carousel_image_size', '_tm-thumb-560-350' );
					...
					echo '<article class="swiper-slide post hentry">';
						if ( $view_dir ) {
							...
							echo '<h5>' . esc_html__( 'View file not found', 'jane_style' ) . '</h5>';
						}
					echo '</article>';

				endwhile;
			} else {
				echo '<h5>' . esc_html__( 'Posts not found', 'jane_style' ) . '</h5>';
			}
		// Reset the query.
		wp_reset_postdata();
	...
	public function get_image( $id, $size, $placeholder_attr, $only_url = false ) {
		...
		$default_placeholder_attr = apply_filters( 'jane_style_carousel_placeholder_default_args',
			array(
				'width'			=> 900,
				'height'		=> 500,
				'background'	=> '000',
	...
	public function get_post_comments( $post_id ) {
		...
		$title_comments =  sprintf( _n( '%s' . __( " comment", "jane_style" ) , '%s'. __( " comments", "jane_style" ), $comments, 'jane_style' ), $comments );
		$comments = ( ! empty( $comments ) ) ? sprintf( '<span class="post-comments-link"><a href="%1$s">%2$s</a></span>', esc_url( get_comments_link() ), $title_comments ) : esc_html__( '0 comments', 'jane_style' );
...
add_action( 'widgets_init', 'tm_register_carousel_widgets' );
function tm_register_carousel_widgets() {
	register_widget( 'jane_style_Carousel_Widget' );
}