 ...
 * @return string
 */
function jane_style_set_sidebar_position( $position ) {
	$queried_obj = apply_filters( 'jane_style_queried_object_id', false );
	if ( ! $queried_obj ) {
		if ( ! is_singular() ) {
			return $position;
		}
...
add_filter( 'theme_mod_sidebar_position', 'jane_style_set_sidebar_position' );
 ...
 * @return string
 */
function jane_style_render_macros( $string ) {

	$macros = apply_filters( 'jane_style_data_macros', array(
		'/%%year%%/' => date( 'Y' ),
 ...
 * @return string
 */
function jane_style_render_icons( $content ) {

	$icons     = jane_style_get_render_icons_set();
	$icons_set = implode( '|', array_keys( $icons ) );

	$regex = '/icon:(' . $icons_set . ')?:?([a-zA-Z0-9-_]+)/';

	return preg_replace_callback( $regex, 'jane_style_render_icons_callback', $content );
}
 ...
 * @return string
 */
function jane_style_render_icons_callback( $matches ) {
 ...
 * @return array
 */
function jane_style_get_render_icons_set() {
	return apply_filters( 'jane_style_render_icons_set', array(
 ...
 * @return string
 */
function jane_style_render_theme_url( $url ) {
 ...
 * @return string
 */
function jane_style_get_thumbnail_size() {
	...
	return 'jane_style-post-thumbnail-large';
}
 ...
 */
function jane_style_get_image_id_by_url( $image_src ) {
	global $wpdb;
...
function jane_style_post_formats_gallery() {
	if ( ! in_array( get_theme_mod( 'blog_layout_type' ), array( 'masonry-2-cols', 'masonry-3-cols' ) ) ) {
		return do_action( 'cherry_post_format_gallery', array(
			'size' => jane_style_get_thumbnail_size(),
		) );
	}
	$images = jane_style_theme()->get_core()->modules['cherry-post-formats-api']->get_gallery_images(false);
