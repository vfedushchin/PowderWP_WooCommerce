...
class Jane_Style_WC_Widget_Product_Categories extends WC_Widget_Product_Categories {
	/**
	 * Category ancestors.
	 *
	 * @var array
		...
		include_once( WC()->plugin_path() . '/includes/walkers/class-product-cat-list-walker.php' );
			...
			foreach ($cats as $cat) {
				...
				echo '<li class="col-lg-3 col-md-3 col-sm-6 col-xs-6"><a href="' . get_term_link( $cat ) . '">
						...
						do_action( 'jane_style_category_cat_name' , printf( '<h5>%s</h5>', $cat_name ), $cat_name );
						if ( $count ) {
							echo ' <span class="count"> ' . $cat->count . ' <em> ' .__('products', 'jane_style'). ' </em></span>';
						}
						echo "</div>";
				echo '</a></li>';
			}
