<?php
/**
 * Widget powder subscribe and follow
 *
 * @package powder
 */

class Powder_Subscribe_Follow_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_cssclass    = 'powder widget-subscribe';
		$this->widget_description = __( 'Display subscribe form and follow links.', 'powder' );
		$this->widget_id          = 'powder_widget_subscribe_follow';
		$this->widget_name        = __( 'Subscribe and Follow', 'powder' );
		$this->settings           = array(
			'enable_subscribe' => array(
				...
				'options' => array(
					'enable_subscribe' => __( 'Enable Subscribe Box', 'powder' ),
				),
			),
			'subscribe_title' => array(
				'type'  => 'text',
				'value' => __( 'Subscribe', 'powder' ),
				'label' => __( 'Subscribe Title', 'powder' ),
			),
			'subscribe_message' => array(
				'type'  => 'textarea',
				'label' => __( 'Subscribe text message', 'powder' ),
			),
			'subscribe_input' => array(
				'type'  => 'text',
				'value' => __( 'Enter your email', 'powder' ),
				'label' => __( 'Subscribe input placeholder', 'powder' ),
			),
			'subscribe_submit' => array(
				'type'  => 'text',
				'value' => __( 'Subscribe', 'powder' ),
				'label' => __( 'Subscribe submit label', 'powder' ),
			),
			'subscribe_success' => array(
				'type'  => 'text',
				'value' => __( 'You successfully subscribed', 'powder' ),
				'label' => __( 'Subscribe success', 'powder' ),
			),
			'enable_follow' => array(
				'type'   => 'checkbox',
				'value'  => array(
				...
				'options' => array(
					'enable_follow' => __( 'Enable Follow Box', 'powder' ),
				),
			),
			'follow_title' => array(
				'type'  => 'text',
				'value' => __( 'Follow', 'powder' ),
				'label' => __( 'Follow Title', 'powder' ),
			),
			'follow_message' => array(
				'type'  => 'textarea',
				'label' => __( 'Follow text message', 'powder' ),
			),
		);

		add_action( 'wp_ajax_powder_subscribe', array( $this, 'process_subscribe' ) );
		add_action( 'wp_ajax_nopriv_powder_subscribe', array( $this, 'process_subscribe' ) );

		$this->errors = array(
			'nonce'     => esc_html__( 'Security validation failed', 'powder' ),
			'mail'      => esc_html__( 'Please, provide valid mail', 'powder' ),
			'mailchimp' => esc_html__( 'Please, set up MailChimp API key and List ID', 'powder' ),
			'internal'   => esc_html__( 'Internal error. Please, try again later', 'powder' ),
		);
	...
	public function widget( $args, $instance ) {
		...
		if ( $subscribe_enabled && $subscribe_template && $api_key && $list_id ) {
			...
			_e( 'Please set up MailChimp API key and List ID', 'powder' );
		}
	...
	public function get_social_nav() {
		return powder_get_social_list( 'widget' );
	}
	...
	public function process_subscribe() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'powder_subscribe' ) ) {
...
add_action( 'widgets_init', 'powder_register_subscribe_follow_widgets' );
function powder_register_subscribe_follow_widgets() {
	register_widget( 'powder_Subscribe_Follow_Widget' );
}
