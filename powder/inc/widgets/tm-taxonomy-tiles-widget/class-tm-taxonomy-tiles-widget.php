...
if ( !class_exists( 'Powder_Taxonomy_Tiles_Widget' ) ) {

	/**
	 * Taxonomy Tiles Widget
	 */
	class Powder_Taxonomy_Tiles_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name			= esc_html__( 'Taxonomy Tiles Widget', 'powder' );
			$this->widget_description	= esc_html__( 'This widget displays images from taxonomy.', 'powder' );
			...
			$this->settings = array(
				'title'	=> array(
					'type'				=> 'text',
					'value'				=> '',
					'label'				=> esc_html__( 'Widget title', 'powder' ),
				),
				'terms_type' => array(
					'type'				=> 'radio',
					'value'				=> 'category',
				...
				'category'=> array(
					...
					'label'				=> esc_html__( 'Select category to show', 'powder' ),
				),
				'post_tag' => array(
					'type'				=> 'select',
					'multiple'			=> true,
					...
					'label'				=> esc_html__( 'Select tags to show', 'powder' ),
				),
				'description_length' => array(
					...
					'min_value'			=> '0',
					'step_value'		=> '1',
					'label'				=> esc_html__( 'Description words length ( Set 0 to hide description. )', 'powder' ),
				),
				'show_post_count' => array(
					...
					'options'		=> array(
						'show_post_count_check' => esc_html__( 'Show post count', 'powder' ),
					),
				),
				'columns_number' => array(
					...
					'min_value'			=> '1',
					'step_value'		=> '1',
					'label'				=> esc_html__( 'Columns number', 'powder' ),
				),
				'first_item' => array(
					'type'				=> 'checkbox',
					'value'			=> array(
					...
					'options'		=> array(
						'first_item_check' => esc_html__( 'Tiles first item', 'powder' ),
					),
				),
				'items_padding' => array(
					'type'				=> 'stepper',
					...
					'min_value'			=> '2',
					'step_value'		=> '1',
					'label'				=> esc_html__( 'Items padding ( size in pixels )', 'powder' ),
				),
			);
		...
		public function get_terms_list ( $tax = 'category', $args = array( 'hide_empty' => 0, 'hierarchical' => 0 ) ) {
			...
			if ( $terms ){
				foreach ( $terms as $term ) {
					$output_terms[ $term->term_id ] = $term->name . sprintf( _n( ' ( 1 post )', ' ( %s posts )', $term->count, 'powder' ), $term->count );
				}
			}
		...
		public function get_term_post_count( $term, $visible ) {
			return ( 'false' === $visible ) ? '' : '<span class="post-count">' . sprintf( _n( '1 post', '%s posts', $term->count, 'powder' ), $term->count ) . '</span>';
		}
		...
		public function get_term_description( $term, $lenght ) {
			...
			return ( 0 === $lenght || !$term->description ) ? '' : '<p class="post-desc">' . wp_trim_words( $term->description, $lenght, esc_html__( ' ...', 'powder' ) ) . '</p>';
		}
		...
		public function get_term_image( $term, $image_size ) {
			...
			if( $image ){
				...
				$placeholder_attr = apply_filters( 'powder_taxonomy_tiles_widget_placeholder_default_args',
					array(
						'width'			=> $size['width'],
						'height'		=> $size['height'],
						'background'	=> '000',
		...
		public function widget( $args, $instance ) {
			...
			if ( isset( $terms ) && $terms ) {
				...
				$tiles_item_number = apply_filters( 'powder_taxonomy_tiles_item_number', 0 );
				...
				echo apply_filters( 'powder_taxonomy_tiles_widget_before', '<div class="row grid-columns columns-number-' . $columns_number . '" ' . $row_inline_style . '>' );
				...
				foreach ( $terms as $term_key => $term ) {
					...
					$image = $this->get_term_image( $term, apply_filters( 'powder_taxonomy_tiles_widget_size', '_tm-thumb-m' ) );
				...
				echo apply_filters( 'powder_taxonomy_tiles_widget_after', '</div>' );
			}
	...
	add_action( 'widgets_init', 'powder_register_taxonomy_tiles_widget' );
	function powder_register_taxonomy_tiles_widget() {
		register_widget( 'powder_Taxonomy_Tiles_Widget' );
	}
