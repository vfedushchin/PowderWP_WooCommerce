...
class Jane_style_Instagram_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Display a list of photos from Instagram network.', 'jane_style' );
		$this->widget_id          = 'jane_style_widget_instagram';
		$this->widget_name        = esc_html__( 'Instagram', 'jane_style' );
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',
				'value' => esc_html__( 'Instagram', 'jane_style' ),
				'label' => esc_html__( 'Title', 'jane_style' ),
			),
			'client_id' => array(
				'type'  => 'text',
				'value' => '',
				'label' => esc_html__( 'Client ID', 'jane_style' ),
			),
			'tag' => array(
				'type'  => 'text',
				'value' => '',
				'label' => esc_html__( 'Hashtag (enter without `#` symbol)', 'jane_style' ),
			),
			'image_counter' => array(
				'type'       => 'stepper',
				'value'      => '6',
				...
				'min_value'  => '1',
				'step_value' => '1',
				'label'      => esc_html__( 'Number of photos', 'jane_style' ),
			),
			'display_caption' => array(
				...
				'options' => array(
					'display_caption_check' => esc_html__( 'Caption', 'jane_style' ),
				),
			),
			'display_date' => array(
				'type'  => 'checkbox',
				...
				'options' => array(
					'display_date_check' => esc_html__( 'Date', 'jane_style' ),
				),
			),
		);
	...
	public function widget( $args, $instance ) {
		...
		if ( empty( $instance['client_id'] ) ) {
			return print $args['before_widget'] . __( 'Please, enter your Instagram CLIENT ID.', 'jane_style' ) . $args['after_widget'];
		}

		if ( empty( $instance['tag'] ) ) {
			return print $args['before_widget'] . __( 'Please, enter #hashtag.', 'jane_style' ) . $args['after_widget'];
		}
		...
		if ( ! $photos ) {
			return print $args['before_widget'] . __( 'No photos. Maybe you entered a invalid CLIENT ID or hashtag.', 'jane_style' ) . $args['after_widget'];
		}
		...
		printf( '<div class="%s">',
			join( ' ', apply_filters( 'jane_style_instagram_widget_wrapper_class', array( 'instagram__items' ) ) )
		);
		...
		foreach ( $result['data'] as $photo ) {
			...
			if ( in_array( 'caption', $config ) ) {
				...
				$_photo['caption'] = substr(
					$caption,
					0,
					apply_filters( 'jane_style_instagram_widget_caption_length', 10 )
				);
			}
...
add_action( 'widgets_init', 'jane_style_register_instagram_widget' );
function jane_style_register_instagram_widget() {
	register_widget( 'jane_style_Instagram_Widget' );
}
